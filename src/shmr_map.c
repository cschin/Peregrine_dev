...
void process_map(
		char * refdb_file_path,
		char * seqdb_file_path,
		mm128_v * ref_mmers,
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv, "r:m:p:l:t:c:b:")) != -1) {
		switch (c) {
			...
			case '?':
				if (optopt == 'r') {
					fprintf (stderr, "Option -%c not specified, using 'ref' as the ref sequence db prefix\n", optopt);
				}
				if (optopt == 'p') {
	...
	int written;
	written = snprintf(ref_idx_file_path, sizeof(ref_idx_file_path), "%s.idx", refdb_prefix);
	...
	written = snprintf(refdb_file_path, sizeof(seqdb_file_path), "%s.seqdb", refdb_prefix);

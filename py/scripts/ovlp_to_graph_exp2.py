...
def find_bundle(ug, u_edge_data, start_node, depth_cutoff, width_cutoff, length_cutoff):
    ...
    while 1:
        ...
        if depth > 50 and width > width_cutoff:
            converage = False
            break
    ...
    for p in list(branch_nodes):
        if ug.out_degree(p) > 1:
            coverage, data, data_r = find_bundle(
                ug, u_edge_data, p, 256, 256, 2500000)
            if coverage == True:
...
def identify_spurs(ug, u_edge_data, spur_len):
    ...
    while len(s_candidates) > 0:
        ...
        if ug2.in_degree(n) != 0:
            continue
        n_ego_graph = nx.ego_graph(ug2, n, radius=20)
        n_ego_node_set = set(n_ego_graph.nodes())
        #for b_node in n_ego_graph.nodes():
        dfs_sorted_nodes = []
        for k, v in nx.bfs_successors(n_ego_graph, n):
            dfs_sorted_nodes.extend(v)

        for b_node in dfs_sorted_nodes:
                    ...
                    if type(edges) is list:
                        ...
                        for e in edges:
                            if type(e) is str:
                                ...
                                r_edges.append( (reverse_end(e[1]), reverse_end(e[0]), reverse_end(e[2]) ) )
                    try:
                        print("edge removed:", s, t)
                        print("edge removed:", rs, rt)
                        ...
                        if ug2.in_degree(t) == 0 or ug2.in_degree(rt) == 0:
                            s_candidates.add(t)
                            s_candidates.add(rt)
                        if ug2.in_degree(s) == 0 or ug2.in_degree(rs) == 0:
                            s_candidates.add(s)
                            s_candidates.add(rs)
                    except Exception:
                        pass
...
def construct_c_paths_from_utgs_subgraph(ug, u_edge_data):
    ...
    for nodes in nx.weakly_connected_components(ug):
        subG = ug.subgraph(nodes).copy()
        # source_nodes = [_ for _ in nodes if subG.in_degree(_) == 0]
        # sink_nodes = [_ for _ in nodes if subG.out_degree(_) == 0]
        edges_to_remove = [ (s, t, v) for s, t, v in subG.edges(keys=True)
                        if subG.in_degree(s) >= 2 and  subG.out_degree(t) >= 2]
        ...
        for s in nodes_to_remove:
            subG.remove_node(s)
        for nodes2 in nx.weakly_connected_components(subG):
            subGraphs.append(subG.subgraph(nodes2).copy())

    edges_to_remove = set()
    c_paths = []
    for g in subGraphs:
        nodes = g.nodes()
        source_nodes = [_ for _ in nodes if g.in_degree(_) == 0]
        sink_nodes = [_ for _ in nodes if g.out_degree(_) == 0]
        all_shorest_paths = []
        for s in source_nodes:
            path_lengths = nx.single_source_dijkstra_path_length(g, s)
            for t in sink_nodes:
                if t in path_lengths:
                    all_shorest_paths.append( (path_lengths[t], s, t) )
        all_shorest_paths.sort()
...
def construct_c_paths_from_utgs(ug_in, u_edge_data, sg):
    # Side-effects: None, I think.

    c_paths, edges_to_remove = \
        construct_c_paths_from_utgs_subgraph(ug_in, u_edge_data)
    ...
    while free_edges:
        ...
        for s, t, v in ug.out_edges(n, keys=True):
            ...
            c_paths.append((path_start, path_key, path_end,
...
def ovlp_to_graph(args):
    ...
    compound_path_file = open("compound_path", "w")
    ...
    c_paths.sort(key=lambda x: -x[3])

    for path_start, path_key, path_end, p_len, p_score, path, n_edges in c_paths:
        length = 0
        score = 0
        length_r = 0
        score_r = 0

...
from peregrine.utils import get_align_range, get_cns_from_reads
...
def get_kmer_count(rid, ovlps, k=31):
    ...
    for rid2 in ovlps[rid]:
        ...
        if ovlps[rid][rid2][1] == 0:
            ...
            if direction == 1:
                s, e = l - e, l - s
            seq0 = read_sdb.get_subseq_by_rid(rid2, s, e, direction=direction)
        else:
            ref_s = int(rec[9])
            ref_e = int(rec[10])
            ref_l = int(rec[11])
        ...
        seq, _ = hp_compress(seq0)
        support_reads[rid2] = seq, seq0, coor_map[ref_s], coor_map[ref_e-1], direction, ovlps[rid][rid2][1]
...
if __name__ == "__main__":
    ...
    k = 31
    for rid in ovlps:
        seqs = [read_sdb.get_subseq_by_rid(rid)]
        count, support_reads, cov, markers = get_kmer_count(rid, ovlps, k=k)
        if count is None: # filter reads with un-covered bases
            # print("E ", rid)
            continue
        for rid1 in ovlps[rid]:
            #if ovlps[rid][rid1][1] == 0:
                #continue
            seq1, seq1_, ref_s, ref_e, direction, d = support_reads[rid1]
        ...
        cns_seq = get_cns_from_reads(seqs, sort_reads=False, best_n=50, min_cov=0)
        print(">" + read_sdb.index_data[rid].rname)
